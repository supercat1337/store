...
import { compareAny, debounce } from "./../src/helpers.js";
...
test("setItems #1 (try to set computed)", (t) => {
    ...
    store.createComputedItem("c", (store) => {
...
test("subscribe #3 (computed, with changes)", (t) => {
    ...
    store.createComputedItem("d", (store) => {
...
test("subscribe #6 (with changes, with array values)", (t) => {
    ...
    store.createCollectionItem("a", [23]);
    store.createAtomItem("b", 2);
...
test("subscribe #8-1 (with changes, with undefined values)", (t) => {
    var store = new Store();
    store.setItems({ a: undefined, b: 2 });
...
test("subscribe #9 (atom, setCompareFunction)", (t) => {
    var store = new Store();
    store.createAtomItem("a", { prop: 1, data: { qwe: 900 } });
    store.createAtomItem("b", 2);
...
test("onChangeAny #5 (with Collections)", (t) => {
    var store = new Store();
    var a = store.createCollection(/** @type {number[]} */ ([]));
    var b = store.createCollection(/** @type {number[]} */ ([]));
...
test("createComputedItem #2 (sealed)", (t) => {
    ...
    var is_created = store.createComputedItem("c", (store) => {
        return store.getItem("a") + store.getItem("b");
    });
...
test("createComputedItem #3 (not atoms, no deps)", (t) => {
    ...
    try {
        var is_created = store.createComputedItem("d", (store) => {
            return 123;
        });
...
test("createComputedItem #4 (not valid name)", (t) => {
    ...
    try {
        store.createComputedItem("c djdj", (store) => {
            return store.getItem("a") + store.getItem("b");
        });
...
test("createComputedItem #6 (with error)", (t) => {
    var store = new Store();
    store.setItems({ a: "abcdef", b: "ghijk" });
...
test("recalcComputed #3 (with subscribers)", (t) => {
    ...
    store.createCollectionItem("b", [1, 2, 3]);
...
test("recalcComputed #5 (with subscribers, with custom compare function)", (t) => {
    var store = new Store();
    store.setItems({ a: 1, b: 1 });

    var foo = 0;

    var c = store.createComputed((store) => {
        var result = {
            payload: store.getItem("a") + store.getItem("b"),
            id: 100,
        };
    ...
    c.setCompareFunction((old_value, value) => {
        if (!old_value) return true;
...
test("#setCollectionItem #1 (with changes)", (t) => {
    ...
    store.subscribe("c", (details) => {
        store.log(
            "collection item is changed. (property: " +
                details.property +
                ", value: " +
                details.value +
                ")"
...
test("asObject #5 (keys)", (t) => {
    ...
    var equals = compareAny(keys, ["a", "b", "c"]);
...
test("observeObject #2", (t) => {
    ...
    var store = new Store();
    var c = store.createCollection(/** @type {string[]} */ ([]), "c");

    store.setItems({ a: 1, b: 2, f: 5 });
    ...
    if (
        ...
        sample.a == 2 &&
        working &&
        sample.store.isAtomItem("b") &&
        sample.store.isAtomItem("d") &&
        !sample.store.isAtomItem("e")
    ) {
        t.pass();
    } else {
        t.fail();
...
test("getUsedItems", (t) => {
    ...
    if (
        ...
        result.items.indexOf("a") > -1 &&
        result.items.indexOf("b") > -1
    ) {
        t.pass();
    } else {
        t.fail();
...
test("collection (value, content)", (t) => {
    ...
    if (
        ...
        collection.value[3] === collection.content[3]
    ) {
        t.pass();
    } else {
        t.fail();
